/*  Vivien Bautista, Redgy Canos, Ann Dinh
	cssc0601
	team missouri
	file_parser.cc
	cs530, Fall 2016
...
using namespace std;
	...
	void file_parser::throw_exception(string s) {
		ostr << "<<ERROR>> Line " << numlines << ": " << s;
		throw file_parse_exception(ostr.str());
	} // End throw_exception
	
	
	// Validates the label
	void file_parser::valid_label(string s) {
		if(is_label(s) == 0) 
			throw_exception("token in column[0] is not a label");

		if(isalpha(s.at(0)) == 0) {
			if(s.compare("#minclude") != 0) 
	...
	string file_parser::remove_space(string s) {
		if(isspace(s.at(s.length()-1)) != 0)
	...
	void file_parser::tokenize(int i) {
		...
		while(string::npos != pos || string::npos != last_pos) {
			string str = s.substr(last_pos, pos-last_pos);	// Grab string
	...
	void file_parser::read_file() {
		...
		if(!in) throw file_parse_exception("<<ERROR>> Could not open the file");
		while(!in.eof()) {
			getline(in, line);
